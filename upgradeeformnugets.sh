...
if (( "$GIT_STATUS" > 0 )); then
	...
	EFORM_PLUGINBASE_VERSION=`dotnet list package | grep 'Microting.eFormTrashInspectionBase' | cut -c64-71`
	COMMIT_MESSAGE="Updating"$'\n'"- Microting.eForm to ${EFORM_VERSION}"$'\n'"- Microting.eFormApi.BasePn to ${EFORM_BASEPN_VERSION}"$'\n'"- Microting.eFormTrashInspectionBase to ${EFORM_PLUGINBASE_VERSION}"
