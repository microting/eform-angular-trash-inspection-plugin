...
def main():
    ...
    if int(get_git_status()) > 0:
        ...
        os.chdir("eFormAPI/Plugins/TrashInspection.Pn/TrashInspection.Pn")
        packages = [
            'Microting.eForm', 'Microting.eFormApi.BasePn', 'Microting.eFormTrashInspectionBase',
            'Microsoft.AspNetCore.Identity.EntityFrameworkCore'
        ]
        process_repository('TrashInspection.Pn.csproj', packages, 'eform-angular-trash-inspection-plugin')

        os.chdir("..")
        os.chdir("TrashInspection.Pn.Test")
        packages = [
            'Microsoft.NET.Test.Sdk', 'NSubstitute', 'NUnit', 'NUnit3TestAdapter',
            'NUnit.Analyzers', 'Testcontainers', 'Testcontainers.Mariadb'
        ]
        process_repository('TrashInspection.Pn.Test.csproj', packages, 'eform-angular-trash-inspection-plugin')
        ...
        if new_number_of_commits > current_number_of_commits:
            ...
            os.chdir("../../../..")
            run_command(f"github_changelog_generator -u microting -p eform-angular-trash-inspection-plugin -t {os.getenv('CHANGELOG_GITHUB_TOKEN')}", shell=True)
